...
NVCF_PEXEC_MODELS = {
    ...
    "glm5_1_nv": {
        ...
        # 此条目保留仅作 MODEL_MAP 别名兼容, 实际请求会走 FALLBACK_GRAPH → dsv4p_nv.
        # 推荐改用 glm5_2_nv (ai-glm-5_2, 3b9748d8, ACTIVE).
    ...
    "glm5_2_nv": {
        ...
        # 抓包探测: glm-5.2 思考触发参数 = chat_template_kwargs.enable_thinking (与 glm-5.1 同)
...
FALLBACK_GRAPH = {
    ...
    # 2026-07-03: NVCF 全系下架 glm-5_1 (6155636e/af904f0c/46f4fb53 均 INACTIVE, pexec 404).
    # opencode 走 glm5_1_nv 会持续 502. 破例启用跨 model fallback → dsv4p_nv (仍 ACTIVE).
    # 首选 6155636e 健康度跌破阈值后, 自动切到 dsv4p_nv, 让 opencode 在 glm5.1 平台下线期间仍可用.
    "glm5_1_nv": ["dsv4p_nv"],
    # glm5_2_nv 主力 (openclaw 默认); 若 3b9748d8 平台 surge → fallback dsv4p_nv (异模型, 思考质量变).
    "glm5_2_nv": ["dsv4p_nv"],
}
...
def detect_nv_model(model_id: str) -> str:
    ...
    Returns: one of kimi_nv / dsv4p_nv / glm5_1_nv / glm5_2_nv. Falls back to
