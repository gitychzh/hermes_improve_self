...
from .config import (
    ...
    HM_PEER_FALLBACK_ENABLED,
    HM_PEER_FALLBACK_URL,
    HM_PEER_FALLBACK_TIMEOUT,
)
...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle_openai_nv(self):
        ...
        if not result.success:
            if result.all_keys_exhausted:
                ...
                # 本机单 tier 5 key 全失败(all_tiers_exhausted) 时, 转发到对端 hm40006 同模型.
                ...
                if (HM_PEER_FALLBACK_ENABLED and HM_PEER_FALLBACK_URL
                        and hop_n < 1 and not is_429):
                    _log("HM-PEER-FB", f"local all_tiers_exhausted (model={mapped_model}), "
                                       f"attempting peer fallback to {HM_PEER_FALLBACK_URL}")
                    ok = self._peer_fallback(body, mapped_model, is_stream, metrics)
                    if ok:
                        metrics["peer_fallback_used"] = True
                        _log_metrics(metrics)
                        return
                    _log("HM-PEER-FB", f"peer fallback FAILED for model={mapped_model}, "
                                       f"returning local 502")
                elif hop_n >= 1:
                    _log("HM-PEER-FB", f"peer-originated request (hop={hop_n}) also "
    ...
    def _peer_fallback(self, body, mapped_model, is_stream, metrics):
        """Forward request to peer hm40006 (same model) when local all_tiers_exhausted.
        ...
        Auth: sends Authorization: Bearer <HM_GATEWAY_API_KEY> so peer's _check_auth passes.
        ...
        try:
            ...
            p = urlparse(HM_PEER_FALLBACK_URL)
            host = p.hostname
            port = p.port or 40006
            peer_path = p.path.rstrip("/") + "/v1/chat/completions"
        except Exception as e:
            _log("HM-PEER-FB", f"bad HM_PEER_FALLBACK_URL={HM_PEER_FALLBACK_URL}: {e}")
            return False
        ...
        if HM_GATEWAY_API_KEY:
            fwd_headers["Authorization"] = f"Bearer {HM_GATEWAY_API_KEY}"
        ...
        try:
            ...
            _log("HM-PEER-FB", f"peer connect/request failed after {elapsed_ms}ms: "
                               f"{type(e).__name__}: {e}")
            if peer_conn:
                try: peer_conn.close()
        ...
        if resp.status >= 500 or resp.status == 429:
            ...
            _log("HM-PEER-FB", f"peer returned {resp.status} after {elapsed_ms}ms, "
        ...
        try:
            ...
            _log("HM-PEER-FB", f"peer fallback OK: status={resp.status} "
                               f"bytes={total} ttfb={metrics.get('ttfb_ms')}ms")
            return True
        except Exception as e:
            elapsed_ms = int((time.time() - t_fb_start) * 1000)
            _log("HM-PEER-FB", f"peer relay failed after {elapsed_ms}ms: "
                               f"{type(e).__name__}: {e}")
